...
fn check_custom_zv_dir_warning(app: &App, using_env_var: bool) -> crate::Result<bool> {
    ...
    println!();
    println!("You are using a custom ZV_DIR path: {}", Paint::cyan(&zv_dir.display().to_string()));
    println!("Default path would be: {}", Paint::dim(&default_zv_dir.display().to_string()));
    println!();
    println!("{}", Paint::yellow("Important considerations:"));
    println!("• The ZV_DIR environment variable must be {} available", Paint::red("permanently"));
    println!("• If ZV_DIR is not set in future sessions, zv will break");
    println!("• Make sure ZV_DIR is set in your shell profile or system environment");
    println!("• Temporary ZV_DIR settings (current session only) will cause issues");
    println!();
    println!("This setup will configure zv to use the custom path, but you must ensure");
    println!("ZV_DIR remains permanently available in your environment.");
    println!();
...
pub async fn pre_setup_checks(app: &App, shell: &Shell, using_env_var: bool) -> crate::Result<bool> {
    let zv_dir = app.path();
    ...
    if path_already_in_system {
        ...
        println!("  • {} is already in PATH", Paint::green(&bin_path.display().to_string()));
        println!();
    ...
    println!("  ✗ {} is not in PATH", Paint::red(&bin_path.display().to_string()));
    ...
    if using_env_var {
        ...
        println!("  • ZV_DIR: {} (using default path)", Paint::dim("not needed"));
    }
...
pub async fn setup_shell(app: &mut App, using_env_var: bool, dry_run: bool) -> crate::Result<()> {
    ...
    if dry_run {
        println!("{} zv setup for {} shell...", Paint::yellow("Previewing"), Paint::cyan(&shell.to_string()));
    } else {
...
#[cfg(windows)]
async fn setup_windows_environment(app: &App, using_env_var: bool, dry_run: bool) -> crate::Result<()> {
    ...
    if dry_run {
        println!("{} Windows environment variables...", Paint::yellow("Would set up"));
    } else {
    ...
    if !path_needs_update {
        println!("{}", Paint::green("✓ Windows environment variables are already configured correctly"));
        if using_env_var {
            println!("  • ZV_DIR: {} (using environment variable)", Paint::dim("custom path"));
        } else {
            println!("  • ZV_DIR: {} (using default path)", Paint::dim("not set in registry"));
        }
        println!("  • PATH: {} (already contains zv bin)", Paint::dim("no change needed"));
        return Ok(());
    }
    ...
    if using_env_var {
        println!("  ZV_DIR: {} (using environment variable, not modifying registry)", Paint::dim("custom path"));
    } else {
        println!("  ZV_DIR: {} (using default path, not setting in registry)", Paint::dim("not needed"));
    }
    ...
    if path_needs_update {
        ...
        if !current_path.is_empty() {
            println!("        (to existing PATH with {} entries)", current_path.split(';').count());
        }
    } else {
        println!("  PATH: {} (already contains zv bin)", Paint::dim("no change needed"));
    }
...
#[cfg(not(windows))]
async fn setup_windows_environment(_app: &App, _using_env_var: bool, _dry_run: bool) -> crate::Result<()> {
    unreachable!("Windows setup should not be called on non-Windows platforms")
}

async fn setup_unix_environment(app: &App, shell: &Shell, using_env_var: bool, dry_run: bool) -> crate::Result<()> {
    let zv_dir = app.path();
    
    // Generate shell environment file
    let (env_file, env_content) = shell.export_without_dump(zv_dir, using_env_var);
    ...
    if !env_file_needs_update && !rc_files_need_update {
        println!("{}", Paint::green("✓ Unix shell environment is already configured correctly"));
        println!("  • Environment file: {} (up to date)", Paint::dim(&env_file.display().to_string()));
        println!("  • Shell startup files: {} (already configured)", Paint::dim("no changes needed"));
        return Ok(());
    }
    ...
    if env_file_needs_update {
        if dry_run {
            println!("{} shell environment file: {}", Paint::yellow("Would create/update"), Paint::cyan(&env_file.display().to_string()));
        } else {
            println!("Creating/updating shell environment file: {}", Paint::cyan(&env_file.display().to_string()));
        }
        ...
        println!("Environment file: {} (already up to date)", Paint::dim(&env_file.display().to_string()));
    }
    ...
    if !rc_files.is_empty() && rc_files_need_update {
        ...
        for file in &rc_files {
            ...
            println!("  • {} ({})", Paint::dim(&file.display().to_string()), Paint::yellow(status));
        }
        println!();
    } else if !rc_files_need_update {
        println!("\nShell startup files: {} (already configured)", Paint::dim("no changes needed"));
    }
    ...
    if dry_run {
        ...
        if rc_files_need_update {
            ...
            if !modified_files.is_empty() {
                ...
                println!("\n{}", Paint::yellow("No shell startup files were modified (source line already exists)"));
            }
        } else {
            println!("{}", Paint::green("✓ Shell setup complete (no RC file changes needed)"));
        }
...
async fn add_source_to_file(file_path: &Path, source_line: &str) -> crate::Result<bool> {
    ...
    let source_exists = content.lines().any(|line| {
        ...
        (trimmed.starts_with("source") && trimmed.contains(&source_line.trim().replace("source ", "")))
    });
